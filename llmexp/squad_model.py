...
1. Using the elimination strategy instead of insertion. The reason is that
    the elimination keeps the context from dramatic variations, which makes the 
    effects of the masked words more related to the context.
'''
...
class MaskGeneratingModel(nn.Module):
    ...
    def generate_mask(self, mask_logits, context_mask):
        ...
        """
        mask_probs = 1 - torch.sigmoid(mask_logits) # (batch_size, seq_len)
        mask_probs = torch.clamp(mask_probs, 0.15, 0.85)
    ...
    def loss_func(self, 
        ...
        for perturbed_input_ids, perturbed_attention_mask, user_input_mask in perturbed_samples:
            ...
            reward_loss += (reward * self.bce_loss(mask_prob, 1-user_input_mask, context_mask)).mean()
        ...
        mask_loss = ((0.7 - mean_reward - mask_loss)**2).mean()
